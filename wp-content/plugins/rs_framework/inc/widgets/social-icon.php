	...
	*/
	function __construct() {
		parent::__construct(
			...
			esc_html__( 'RS Social Icon Widget', 'konstruk' ), // Name
			array( 'description' => esc_html__( 'RS social icon widget area', 'konstruk' ), ) // Args
		);
	}
	...
	*/
	public function widget( $args, $instance ) {
		...
		<ul class="footer_social">  
		    ...
		    <?php } ?>    
		    <?php if (!empty($instance['houzz'])) { ?>
		        <li> 
		        <a href="<?php  echo esc_url($instance['houzz'])?> " target="_blank"><span> <i class="fab fa-houzz"></i> </span></a> 
		        </li>
		    <?php } ?>  
		</ul><?php 
	...
	*/
	public function field_generator( $instance ) {
		...
		foreach ( $this->widget_fields as $widget_field ) {
			$widget_value = ! empty( $instance[$widget_field['id']] ) ? $instance[$widget_field['id']] : esc_html__( isset($widget_field['default']), 'konstruk' );
			switch ( $widget_field['type'] ) {
				default:
					$output .= '<p>';
					$output .= '<label for="'.esc_attr( $this->get_field_id( $widget_field['id'] ) ).'">'.esc_attr( $widget_field['label'], 'konstruk' ).':</label> ';
	...
	public function form( $instance ) {
		$title = ! empty( $instance['title'] ) ? $instance['title'] : esc_html__( '', 'konstruk' );
		?>
		<p>
		<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_attr_e( 'Title:', 'konstruk' ); ?></label>
